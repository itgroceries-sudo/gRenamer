...
powershell -noprofile -c "$param='%*';$ScriptPath='%~f0';iex((Get-Content -LiteralPath '%~f0' -Raw))"
exit /b
#>

# =========================================================
#  GRENAM REMOVER ULTIMATE
#  Version: 3.5 Build 04.03.2026 (IEX/IRM Optimized)
#  Framework: IT Groceries Shop (Layout Master)
# =========================================================

[System.Net.ServicePointManager]::SecurityProtocol = [System.Net.SecurityProtocolType]::Tls12

$AppVersion = "3.5 Build 04.03.2026"
$InstallDir = "$env:LOCALAPPDATA\ITG_gRenamer"
$TempScript = "$env:TEMP\v.ps1"
$SelfURL    = "https://raw.githubusercontent.com/itgroceries-sudo/gRenamer/main/v.cmd"
...
# ---------------------------------------------------------
# [3] ELEVATION LOGIC (WinUpdateFix Style + CMD Handler)
# ---------------------------------------------------------
...
if (-not $Silent -and -not $IsAdmin) {
    ...
    try {
        if ($TargetFile -and (Test-Path -LiteralPath $TargetFile)) {
            ...
            # จุดสำคัญที่แก้การหายวับ: ถ้าโหลดเป็น .cmd ต้องเปิดด้วย CMD ไม่ใช่ PowerShell
            if ($TempScript -match '\.(cmd|bat)$') {
                Start-Process -FilePath "$TempScript" -Verb RunAs
            } else {
...
if (!$Silent) {
    ...
    Write-Host "   (V.3.5 Build 04.03.2026 : INIT)      " -ForegroundColor Green
