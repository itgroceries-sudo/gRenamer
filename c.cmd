...
:: ระบบขอสิทธิ์ Admin อัตโนมัติแบบมาตรฐาน
...
#  Version: 4.0 Build 04.03.2026 (The Flawless Edition)
#  Framework: IT Groceries Shop (Layout Master)
# =========================================================

[System.Net.ServicePointManager]::SecurityProtocol = [System.Net.SecurityProtocolType]::Tls12

$AppVersion = "4.0 Build 04.03.2026"
$InstallDir = "$env:LOCALAPPDATA\ITG_gRenamer"
$TempScript = "$env:TEMP\c.ps1"

# ลิงก์ที่ชี้ไปหาไฟล์ต้นฉบับบน GitHub (สำคัญมาก ต้องอัปเดตไฟล์ .cmd ขึ้นลิงก์นี้)
$SelfURL    = "https://raw.githubusercontent.com/itgroceries-sudo/gRenamer/main/c.cmd"
...
# ---------------------------------------------------------
# [3] ELEVATION LOGIC (The Bulletproof WinUpdateFix Method)
# ---------------------------------------------------------
...
if (-not $Silent -and -not $IsAdmin) {
    ...
    try {
        if ($TargetFile -and (Test-Path $TargetFile)) {
            ...
            # บังคับเซฟไฟล์ลง Temp เป็น .cmd ด้วยโหมด UTF8-with-BOM เพื่อให้อ่านภาษาไทยออก
            $Utf8WithBom = New-Object System.Text.UTF8Encoding($True)
            [System.IO.File]::WriteAllText($TempScript, $ScriptContent, $Utf8WithBom)
            
            # รันไฟล์ .cmd นั้นด้วยสิทธิ์ Admin
...
if (!$Silent) {
    ...
    Write-Host "   (V.4.0 Build 04.03.2026 : INIT)      " -ForegroundColor Green
...
Title="gRenam Remover by IT Groceries Shop" Height="750" Width="650" WindowStartupLocation="Manual" ResizeMode="NoResize" Background="#181818" WindowStyle="None" BorderBrush="#4CAF50" BorderThickness="2" FontFamily="Tahoma">
    ...
    <Grid Margin="25">
        ...
        <Grid Grid.Row="3" Margin="0,15,0,8">
            ...
            <StackPanel Grid.Row="1" Orientation="Horizontal" HorizontalAlignment="Center" Margin="0,15,0,0">
                <CheckBox x:Name="ChkAdv" Foreground="#FFCCCCCC" Cursor="Hand">
    ...
    # =========================================================
    # แก้ไขปุ่ม SAVE .CMD (ดาวน์โหลดตัวล่าสุดจาก GitHub แล้วแปลงเป็น Batch)
    # =========================================================
    $BSave.Add_Click({
        Play-Sound "Click"
        $sfd = New-Object System.Windows.Forms.SaveFileDialog
        $sfd.Filter = "Command Script (*.cmd)|*.cmd"
        $sfd.FileName = "gRenam_Remover.cmd"
        ...
        if ($sfd.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) {
            try {
                ...
                # เขียนไฟล์กลับเป็น UTF-8 with BOM ให้ Windows เปิดได้ปกติ
                $Utf8WithBom = New-Object System.Text.UTF8Encoding($True)
                ...
                [System.Windows.MessageBox]::Show("Failed to download or save file: $_", "Error", 0, 16) | Out-Null
            }
        }
    })
