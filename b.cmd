...
:: บังคับให้ PowerShell อ่านไฟล์นี้เป็น UTF-8 เสมอ ป้องกันภาษาไทยพังจนเกิด Syntax Error
powershell -NoProfile -ExecutionPolicy Bypass -Command "$param='%*'; $ScriptPath='%~f0'; iex ((Get-Content -LiteralPath '%~f0' -Encoding UTF8 -Raw))"
exit /b
#>

# =========================================================
#  GRENAM REMOVER ULTIMATE
#  Version: 3.6 Build 04.03.2026 (IEX/IRM Optimized)
#  Framework: IT Groceries Shop (Layout Master)
# =========================================================

[System.Net.ServicePointManager]::SecurityProtocol = [System.Net.SecurityProtocolType]::Tls12

$AppVersion = "3.6 Build 04.03.2026"
$InstallDir = "$env:LOCALAPPDATA\ITG_gRenamer"

# ให้มันสร้างไฟล์หลอกเป็น .ps1 เสมอ เพื่อข้ามปัญหาการรัน Batch ซ้อนในโหมด Admin
$TempScript = "$env:TEMP\b.ps1"
$SelfURL    = "https://raw.githubusercontent.com/itgroceries-sudo/gRenamer/main/b.cmd"
...
if (-not $Silent -and -not $IsAdmin) {
    ...
    try {
        if ($TargetFile -and (Test-Path -LiteralPath $TargetFile)) {
            # ดึงเนื้อหาจากไฟล์ Offline แบบบังคับเป็น UTF-8
            $ScriptContent = Get-Content -LiteralPath $TargetFile -Encoding UTF8 -Raw
        } else {
            # ดึงเนื้อหาจากลิงก์ Online
        ...
        # เขียนไฟล์เป็น .ps1 พร้อมยัด BOM ลงไป เพื่อการันตีว่า PowerShell รอบ 2 จะอ่านออกแน่นอน 100%
        $Utf8WithBom = New-Object System.Text.UTF8Encoding($True)
        [System.IO.File]::WriteAllText($TempScript, $ScriptContent, $Utf8WithBom)
        
        # รันผ่าน .ps1 ตรงๆ ทลายกำแพงการ Crash จาก .cmd
...
if (!$Silent) {
    ...
    Write-Host "   (V.3.6 Build 04.03.2026 : INIT)      " -ForegroundColor Green
    ...
    <Grid Margin="25">
        ...
        <Grid Grid.Row="3" Margin="0,15,0,8">
            ...
            <StackPanel Grid.Column="1" Orientation="Vertical" HorizontalAlignment="Center">
                <Button x:Name="BA" Content="START SCAN" Width="300" Height="50" Background="#2E7D32" Foreground="White" Style="{StaticResource Btn}" Cursor="Hand" FontSize="18"/>
                <CheckBox x:Name="ChkAdv" Foreground="#FFCCCCCC" HorizontalAlignment="Center" Cursor="Hand" Margin="0,12,0,0">
                    <TextBlock x:Name="T_Adv" Text="Advance Mode: Recover hidden orphaned files" FontSize="13" Margin="5,-2,0,0" Padding="0,0,0,5"/>
                </CheckBox>
            </StackPanel>
            
            <StackPanel Grid.Column="2" HorizontalAlignment="Right" VerticalAlignment="Top">
                <Button x:Name="BRefresh" Width="100" Height="50" Background="#1976D2" Foreground="White" Style="{StaticResource Btn}" Cursor="Hand">
                    <StackPanel Orientation="Horizontal">
